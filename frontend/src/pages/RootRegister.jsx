...
import { Lock, Mail, User, Phone, ArrowRight, Eye, EyeOff, KeyRound } from 'lucide-react';
...
const RootRegister = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { secretKey, ...registerData } = formData;
            const response = await api.post('/auth/register-root', registerData, {
                headers: { 'x-root-key': secretKey }
            });
            
            if (response.data.success) {
                toast.success('Root user created successfully!');
                navigate('/login', { state: { isRoot: true } });
            }
        } catch (error) {
        ...
        <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 relative overflow-hidden">
            ...
            >
                <div className="bg-slate-900/40 backdrop-blur-2xl border border-slate-800/60 rounded-3xl p-10 shadow-3xl">
                    ...
                    <h2 className="text-2xl font-black text-center text-white uppercase tracking-[0.2em] mb-8">Root Registration</h2>

                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="relative group">
                            <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-purple-400" />
                            <input
                                type="password"
                                placeholder="Root Secret Key"
                                required
                        ...
                        >
                            {loading ? <ClipLoader color="#fff" size={24} /> : <>Register Root <ArrowRight className="w-5 h-5" /></>}
                        </button>
                    </form>
                </div>
            </motion.div>
